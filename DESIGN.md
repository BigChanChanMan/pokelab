...
- **停用训练家（软删除）** —— 需要新列 + 登录时多一道检查，还要回答
  「停用的训练家算哪一档等级」（答案大概率与「访客不是可设置的等级」同理：
  它是个正交的状态位，不是等级）。真要可逆时再做。
  _2026-09-22 修订_：**硬删除已经做了**（issue #26），从这条里移出。
  原先记的理由是「会立刻撞上 §2 那个至今未答的压力测试」—— 核对下来
  **这条理由不成立**：§6.4「降级不删数据」已经回答了压力测试的后半
  （降级后第 5 支队伍 → 只读），剩下的「访客队伍归属」是**注册时**的事，
  删除碰不到它。内存里的队伍确实会变成孤儿，但它**有界**（重启即清）
  且**不可达**（`getTrainer` 查不到行 → 恒为 GUEST；id 是 UUID，
  同名重注册拿不到旧数据）—— 是垃圾，不是数据泄漏。
  连带删除抽卡记录靠 schema 早就写好的 `onDelete: 'cascade'`，零代码。
  领域规则：**管理员不可删除**，见 `CONTEXT.md` 与 `docs/adr/0002`。
